...
if [ ! -z "$2" ]; then
    ...
    echo -e "ERROR: [storage] missing. (ex.: local-lvm)\n"
    exit -1
fi
if [ ! -z "$3" ]; then
    UBUNTU_DISTRO="$3"
...
setStatus "STEP 8: Retrieve SSH keys from LaunchPad for: ${GITHUB_ID}..."
if wget https://github.com/${GITHUB_ID}.keys -O ./keys; then
    setStatus " - Success." "s"
else
    setStatus " - Error completing step." "f"
    exit -1
